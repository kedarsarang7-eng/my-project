      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (_, snap) {
          if (snap.connectionState == ConnectionState.waiting) {
            return const Padding(padding: EdgeInsets.all(20), child: Column(children: [ShimmerBox(height: 120, radius: 16), SizedBox(height: 16), ShimmerBox(height: 200, radius: 16)]));
          }
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(children: [
                    const Icon(Icons.local_library_rounded, color: Colors.white, size: 36),
                    const SizedBox(width: 16),
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('Library Card', style: TextStyle(color: Colors.white70, fontSize: 12)),
                      Text('${issuedBooks.length} book${issuedBooks.length == 1 ? '' : 's'} issued', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18)),
                    ]),
                  ]),
                ),
                const SectionHeader(title: 'Currently Issued'),
                if (issuedBooks.isEmpty)
                  const EmptyState(message: 'No books currently issued', icon: Icons.book_outlined)
                else
...
class _IssuedBookTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InfoTile(
      ...
      trailing: isOverdue ? const StatusBadge(label: 'OVERDUE', color: AppTheme.error) : null,
    );
  }
}
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (author.isNotEmpty) Text(author, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11)),
        ])),
        Text('$available avail.', style: TextStyle(fontSize: 11, color: available > 0 ? AppTheme.success : AppTheme.error, fontWeight: FontWeight.w600)),
      ]),
    );
  }
}
