...
class _State extends ConsumerState<FeePaymentScreen> {
    ...
    return Scaffold(
      ...
      body: feeAsync.when(
        ...
        error: (e, _) => ErrorState(
            message: e.toString(), onRetry: () => ref.invalidate(feesProvider)),
        data: (data) {
            ...
            child:
              ...
              Container(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Row(children: [
                        Expanded(
                            child: _balanceItem('Total Paid',
                                fmt.format(totalPaid), AppTheme.success)),
                        const SizedBox(width: 12),
                        Expanded(
                            child: _balanceItem('Total Due',
                                fmt.format(totalDue), AppTheme.error)),
                      ]),
                      if (totalDue > 0) ...[
              ...
              if (dues.isNotEmpty) ...[
                ...
                ...dues.map((d) => _FeeItem(
                    ...
                    ref: ref,
                    isPending: true,
                    onPay: (amt) => _initiatePayment(context, amt, fmt))),
              ],
  ...
  Future<void> _initiatePayment(
      BuildContext context, double amount, NumberFormat fmt) async {
    ...
    try {
      ...
      final order = await ref
          .read(schoolRepoProvider)
          .createPaymentOrder({'amount': amount, 'currency': 'INR'});
      ...
      await ref
          .read(schoolRepoProvider)
          .verifyPayment({'orderId': orderId, 'status': 'success'});
...
class _PaymentSheetState extends State<_PaymentSheet> {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('Pay ${widget.fmt.format(widget.amount)}',
              style:
          ...
          SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text('Continue with ${_method.toUpperCase()}',
...
class _MethodTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color:
                selected ? AppTheme.primary.withValues(alpha: 0.08) : AppTheme.cardBg,
...
class _FeeItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final name =
        item['feeName'] ?? item['feeType'] ?? item['description'] ?? 'Fee';
          ...
          Expanded(
              child: Column(
                ...
                if (dueDate.isNotEmpty)
                  Text('Due: $dueDate${isOverdue ? " ⚠ Overdue" : ""}',
                      style: TextStyle(
                          color: isOverdue
                              ? AppTheme.error
