...
class AuthUser {
  ...
  AuthUser({required this.sub, required this.email, required this.name, required this.role, required this.tenantId, this.facultyId});
  ...
  factory AuthUser.fromToken(String token) {
    ...
    return AuthUser(
      ...
      role: (payload['custom:role'] ?? 'teacher').toString().toLowerCase(),
      tenantId: payload['custom:tenantId'] ?? '',
      facultyId: payload['custom:entityId'],
    );
  }

  bool get isTeacher => role == 'teacher' || role == 'faculty';
  bool get isStaff => role == 'staff' || isTeacher;
}
...
class AuthService {
  ...
  Future<AuthUser> signIn(String email, String password) async {
    ...
    if (!authUser.isStaff) throw Exception('Access denied: this app is for teachers and staff only.');
    return authUser;
  }
