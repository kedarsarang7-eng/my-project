...
class _State extends ConsumerState<HomeworkScreen> with SingleTickerProviderStateMixin {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => StatefulBuilder(builder: (ctx, setS) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          TextField(controller: descCtrl, decoration: const InputDecoration(labelText: 'Description / Instructions'), maxLines: 3),
          const SizedBox(height: 12),
          OutlinedButton.icon(
            onPressed: () async {
            ...
            label: Text(dueDate != null ? 'Due: ${DateFormat('d MMM yyyy').format(dueDate!)}' : 'Set Due Date'),
          ),
          const SizedBox(height: 16),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton(
              onPressed: (titleCtrl.text.isEmpty || selectedBatchId == null) ? null : () async {
                Navigator.pop(ctx);
                try {
                  await ref.read(teacherRepoProvider).createHomework({
                    'title': titleCtrl.text,
                    'description': descCtrl.text,
                    'batchId': selectedBatchId,
                    if (dueDate != null) 'dueDate': DateFormat('yyyy-MM-dd').format(dueDate!),
                  });
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Homework assigned!'), backgroundColor: AppTheme.success));
    ...
    return RefreshIndicator(
      ...
      child: async.when(
        ...
        data: (items) => items.isEmpty
            ? const EmptyState(message: 'No homework assigned yet', icon: Icons.assignment_outlined, actionLabel: 'Assign Homework')
            : ListView.builder(
                    ...
                    child: Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          ...
                          StatusBadge(label: '$submitted/$total submitted', color: submitted == total ? AppTheme.success : AppTheme.warning),
                        ]),
                        const SizedBox(height: 6),
                        if (batch.isNotEmpty) Text('Batch: $batch', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                        if (dueDate.isNotEmpty) Text('Due: $dueDate', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                        if (total > 0) ...[
...
class _SubmissionsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(homeworkProvider);
    return async.when(
      loading: () => const Padding(padding: EdgeInsets.all(16), child: ShimmerBox(height: 200)),
      error: (e, _) => ErrorState(message: e.toString()),
      data: (items) {
        if (items.isEmpty) return const EmptyState(message: 'No submissions yet', icon: Icons.assignment_turned_in_outlined);
        final hw = items.first as Map<String, dynamic>;
        return FutureBuilder<List<dynamic>>(
          future: ref.read(teacherRepoProvider).getHomeworkSubmissions(hw['id'] ?? ''),
          builder: (_, snap) {
            if (snap.connectionState == ConnectionState.waiting) return const Padding(padding: EdgeInsets.all(16), child: ShimmerBox(height: 200));
            if (snap.hasError) return ErrorState(message: snap.error.toString());
            final subs = snap.data ?? [];
            if (subs.isEmpty) return const EmptyState(message: 'No submissions yet', icon: Icons.assignment_turned_in_outlined);
                ...
                return Padding(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Text(student, style: const TextStyle(fontWeight: FontWeight.w500))),
                      if (grade != null) Text('Grade: $grade', style: const TextStyle(color: AppTheme.success, fontWeight: FontWeight.w600)),
                      if (grade == null) OutlinedButton(
                        onPressed: () {},
                        style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap, textStyle: const TextStyle(fontSize: 12)),
