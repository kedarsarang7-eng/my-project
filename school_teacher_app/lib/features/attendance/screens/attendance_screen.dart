...
class _AttendanceScreenState extends ConsumerState<AttendanceScreen> {
      ...
      body: batchesAsync.when(
        ...
        data: (batches) => Column(children: [
          ...
          if (_selectedBatchId == null)
            const Expanded(
                child: Center(
                    child: Text('Select a batch to mark attendance',
            ...
            child: DropdownButtonFormField<String>(
              ...
              items: batches.map((b) {
                ...
                return DropdownMenuItem<String>(
                    value: batch['id'],
                    child: Text(batch['name'] ?? batch['id']));
              }).toList(),
              onChanged: (v) {
                final batch =
                    batches.firstWhere((b) => (b as Map)['id'] == v) as Map;
                setState(() {
                  _selectedBatchId = v;
  ...
  Widget _buildStudentList() {
    return FutureBuilder<List<dynamic>>(
      future:
          ref.read(teacherRepoProvider).getStudents(batchId: _selectedBatchId),
      builder: (_, snap) {
        ...
        return Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              _summaryChip(
                  'Present',
                  _statusMap.values.where((v) => v == 'present').length,
                  AppTheme.success),
              const SizedBox(width: 8),
              _summaryChip(
                  'Absent',
                  _statusMap.values.where((v) => v == 'absent').length,
                  AppTheme.error),
              const SizedBox(width: 8),
              _summaryChip(
                  'Leave',
                  _statusMap.values.where((v) => v == 'leave').length,
                  AppTheme.warning),
              const Spacer(),
              TextButton(
                  onPressed: () => setState(() => _statusMap = {
                        for (final s in students)
                          (s as Map)['id'] as String: 'present'
                      }),
  ...
  Color _borderColor(String status) {
    if (status == 'present') return AppTheme.success.withValues(alpha: 0.3);
    if (status == 'absent') return AppTheme.error.withValues(alpha: 0.3);
  ...
  Future<void> _submit(List<dynamic> students) async {
    ...
    try {
      ...
      if (isOffline) {
        await AttendanceOfflineQueue.enqueue(
            records.cast<Map<String, dynamic>>(),
            idempotencyKey: idempotencyKey);
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
                Text('Saved offline — will sync when internet is restored'),
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Saved offline — will retry automatically'),
            backgroundColor: AppTheme.warning));
    } finally {
