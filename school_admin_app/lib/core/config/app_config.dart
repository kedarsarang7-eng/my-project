...
class AppConfig {
  ...
  static const String appRole = 'admin';
  static const String appName = 'EduConnect Admin';

  static Future<void> initialize() async {
    await dotenv.load(fileName: '.env');
    apiBaseUrl = dotenv.env['API_BASE_URL'] ?? 'http://localhost:3000';
    final rawUrl = apiBaseUrl;
    wsBaseUrl = dotenv.env['WS_BASE_URL'] ?? rawUrl.replaceFirst('https://', 'wss://').replaceFirst('http://', 'ws://');
    ...
    if (cognitoClientId.isEmpty || cognitoUserPoolId.isEmpty) throw Exception('Missing Cognito config in .env');
  }
}
