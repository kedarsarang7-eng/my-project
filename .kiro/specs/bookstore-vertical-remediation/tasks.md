...
- [x] 3. Phase 1 — Enforce tenant/vendor isolation on every book-store read and write

  - [x] 3.1 Thread the active Tenant_Id through Book_Repository reads and writes
  ...
  - [x] 3.2 Scope the Book_Store_Handler by the authenticated tenant boundary
  ...
  - [x] 3.3 Reject unresolved-tenant and cross-tenant operations
...
- [x] 5. Phase 2 — Wire the bookStore sidebar, screens, and quick actions (Dev_Flag-gated)

  - [x] 5.1 Add `_getBookStoreSections()` behind an explicit `case BusinessType.bookStore`
  ...
  - [x] 5.2 Map each `book_*` sidebar id to an existing Book_Screen
  ...
  - [x] 5.3 Repair the bookStore dashboard quick actions
  ...
  - [x] 5.4 Route school-orders/consignments through the existing guarded paths; keep GoRouter report-only
...
- [x] 7. Phase 3 — Per-item GST and capped consignment settlement (integer Paise)

  - [x] 7.1 Halt on the tax policy and request explicit confirmation (business decision)
  ...
  - [x] 7.2 Resolve per-item GST from tax class/HSN and reconcile the contradiction
  ...
  - [x] 7.3 Compute the POS tax line and express all money in integer Paise
  ...
  - [x] 7.4 Compute and enforce the consignment Settlement_Cap
...
- [x] 9. Phase 4 — Back catalogue, POS, alerts, and metadata with real tenant-scoped data

  - [x] 9.1 Populate the catalogue from a tenant-scoped Product query
  ...
  - [x] 9.2 Populate the POS grid and wire search to tenant-scoped products
  ...
  - [x] 9.3 Persist author, publisher, and edition on the Product record
  ...
  - [x] 9.4 Derive bookStore dashboard alert counts from real queries
  ...
  - [x] 9.5 Route ISBN lookup and low-stock through deployed endpoints
  ...
  - [x] 9.6 Render empty and error states instead of fabricated values
...
- [x] 11. Phase 5 — Consolidate ISBN validation onto the authoritative checksum validator

  - [x] 11.1 Route all ISBN validation through `BookStoreBusinessRules.isValidIsbn`
  ...
  - [x] 11.2 Enforce ISBN validation in the Add Book dialog
  ...
  - [x] 11.3 Enforce ISBN validation at POS and eliminate the ₹0 placeholder line
...
- [x] 13. Phase 6 — Functional publisher returns and a real loyalty balance

  - [x] 13.1 Make Publisher Returns functional, backed by Book_Repository
  ...
  - [x] 13.2 Derive a real loyalty balance with accrual and bounded redemption
...
- [x] 15. Phase 7 — Migrate Book_Repository onto the Sync_Queue offline-first pattern

  - [x] 15.1 Migrate Book_Repository off direct apiClient calls onto the Sync_Queue
  ...
  - [x] 15.2 Queue offline writes with a pending state and define the publisher-return offline path
  ...
  - [x] 15.3 Flush queued writes idempotently on connectivity restore
  ...
  - [x] 15.4 Retain and retry failed sync entries without discarding them
...
- [x] 17. Phase 8 — Gate money/create writes in-widget and take navigation live

  - [x] 17.1 Add in-widget permission checks on guarded book-store writes
  ...
  - [x] 17.2 Remove the Dev_Flag gating on Phase 8 sign-off
...
- [x] 19. Phase 9 — Capability entries, layout/pagination/lifecycle polish, and backlog gating

  - [x] 19.1 Add gateable capability entries for school orders and consignment
  ...
  - [x] 19.2 Make the POS three-pane layout robust on narrow windows
  ...
  - [x] 19.3 Paginate consignment and school-order loads
    - In `book_repository.dart`, request consignments/school-orders in pages rather than one unpaginated call (F32)
    - _Requirements: 12.3_

  - [x] 19.4 Guard BuildContext after await in async failure branches
  ...
  - [x] 19.5 Omit clearly retail-only sidebar sections
    - In `_getBookStoreSections()`, omit clearly retail-only sections irrelevant to a book store (F34)
    - _Requirements: 12.5_

  - [x] 19.6 Improve accessibility on touched surfaces
  ...
  - [x] 19.7 Flag backlog features as hard stops (business decision)
