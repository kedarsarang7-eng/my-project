...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<StaffUserModel> loginWithBiometrics() async {
    ...
    if (accessToken == null || refreshToken == null) {
      throw Exception(
          'No stored credentials found. Please login with credentials first.');
    }

    // Check if access token is still valid
    if (!isTokenExpired(accessToken)) {
      // Token is valid — decode ID token and return user
      final claims = decodeJwtPayload(idToken ?? accessToken);
      return _staffUserFromClaims(claims);
    }

    // Access token expired — attempt refresh via Cognito
    try {
      // Extract username from token claims for CognitoUser
      final tokenClaims = decodeJwtPayload(accessToken);
      final username = tokenClaims['sub'] as String? ?? '';
      final cognitoUser = CognitoUser(username, _userPool);
      final refreshTokenObj = CognitoRefreshToken(refreshToken);
      final session = await cognitoUser.refreshSession(refreshTokenObj);
      ...
      final claims = decodeJwtPayload(newIdToken);
      return _staffUserFromClaims(claims);
    } catch (e) {
      throw Exception(
        'Token refresh failed. Please login with credentials: $e',
      );
    }
  }
  ...
  Future<bool> isLoggedIn() async {
    ...
    // Validate token expiration — return false if expired or malformed
  ...
  Future<StaffUserModel?> getCurrentUser() async {
    ...
    // Return null if token is expired or malformed
    if (isTokenExpired(idToken)) return null;
  ...
  /// Constructs a [StaffUserModel] from decoded JWT claims (ID token).
  StaffUserModel _staffUserFromClaims(Map<String, dynamic> claims) {
    return StaffUserModel(
      staffId: claims['sub'] as String? ?? '',
      fullName: claims['name'] as String? ?? '',
      role: claims['custom:role'] as String? ?? 'pump_operator',
      pumpStationId: claims['custom:station_id'] as String? ?? '',
      isFirstLogin: false,
      isActive: true,
      permissions: [],
    );
