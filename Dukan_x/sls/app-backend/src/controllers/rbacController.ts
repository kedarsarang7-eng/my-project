    ...
    try {
        const permissions = await query(
            `SELECT id, category, display_name, description, sort_order, is_sensitive
             FROM permissions ORDER BY category, sort_order`
        );
    ...
    try {
        ...
        const permissions = await query(
            `SELECT rp.permission_id, p.category, p.display_name, p.description, p.is_sensitive
             ...
             WHERE rp.role_id = $1 AND rp.tenant_id = $2 ORDER BY p.category, p.sort_order`,
            [roleId, tenantId]
        );
...
router.put('/roles/:roleId/permissions', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        const role = await queryOne<{ name: string }>(`SELECT name FROM roles WHERE id = $1 AND tenant_id = $2`, [roleId, tenantId]);
        if (!role) { res.status(404).json({ error: 'Role not found' }); return; }
        if (role.name === 'owner') { res.status(403).json({ error: 'Cannot modify owner role permissions' }); return; }

        await withTenant(tenantId, async (client) => {
            await client.query(`DELETE FROM role_permissions WHERE role_id = $1 AND tenant_id = $2`, [roleId, tenantId]);
            if (permission_ids.length > 0) {
                const values = permission_ids.map(
                    (pid: string, i: number) => `(gen_random_uuid(), $1, $${i + 3}, $2, NOW(), $${permission_ids.length + 3})`
                ).join(', ');
                await client.query(
                    `INSERT INTO role_permissions (id, role_id, permission_id, tenant_id, granted_at, granted_by) VALUES ${values}`,
...
router.post('/roles', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        if (!name || !display_name) { res.status(400).json({ error: 'name and display_name are required' }); return; }

        const role = await queryOne<{ id: string }>(
            `INSERT INTO roles (tenant_id, name, display_name, description, is_system) VALUES ($1, $2, $3, $4, FALSE) RETURNING id`,
        ...
        if (role && Array.isArray(permission_ids) && permission_ids.length > 0) {
            ...
            for (const pid of permission_ids) {
                await query(
                    `INSERT INTO role_permissions (role_id, permission_id, tenant_id, granted_by) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`,
        ...
        if (error.constraint?.includes('roles_tenant_id_name_key')) {
            res.status(409).json({ error: 'A role with this name already exists' }); return;
        }
    ...
    try {
        ...
        const staff = await query(
            `SELECT sm.id, sm.cognito_sub, sm.email, sm.phone, sm.name,
                    sm.is_active, sm.invite_status, sm.last_login_at, sm.created_at, sm.updated_at,
                    r.id AS role_id, r.name AS role_name, r.display_name AS role_display_name
             FROM staff_members sm JOIN roles r ON r.id = sm.role_id
             WHERE sm.tenant_id = $1 ORDER BY sm.is_active DESC, sm.name`,
            [tenantId]
        );
        res.json({ staff });
    } catch (error: any) {
...
router.post('/staff', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        if (!name || !role_id) { res.status(400).json({ error: 'name and role_id are required' }); return; }

        const role = await queryOne<{ name: string }>(`SELECT name FROM roles WHERE id = $1 AND tenant_id = $2`, [role_id, tenantId]);
        if (!role) { res.status(404).json({ error: 'Role not found for this business' }); return; }
        if (role.name === 'owner') { res.status(403).json({ error: 'Cannot assign owner role to staff' }); return; }
        ...
        if (error.constraint) { res.status(409).json({ error: 'A staff member with this email or phone already exists' }); return; }
...
router.put('/staff/:staffId', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        if (role_id !== undefined) {
            const role = await queryOne<{ name: string }>(`SELECT name FROM roles WHERE id = $1 AND tenant_id = $2`, [role_id, tenantId]);
            if (!role) { res.status(404).json({ error: 'Role not found' }); return; }
            if (role.name === 'owner') { res.status(403).json({ error: 'Cannot assign owner role' }); return; }
            updates.push(`role_id = $${idx++}`); params.push(role_id);
        }
        ...
        const staff = await queryOne(
            `UPDATE staff_members SET ${updates.join(', ')} WHERE id = $${idx++} AND tenant_id = $${idx} RETURNING id, name, role_id, is_active`,
            params
        );
...
router.delete('/staff/:staffId', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        const result = await queryOne(
            `UPDATE staff_members SET is_active = FALSE, invite_status = 'revoked', updated_at = NOW()
             WHERE id = $1 AND tenant_id = $2 RETURNING id, name`,
            [staffId, tenantId]
        );
        if (!result) { res.status(404).json({ error: 'Staff member not found' }); return; }
...
router.get('/staff/:staffId/permissions', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        const staff = await queryOne<{ cognito_sub: string; role_id: string }>(
            `SELECT cognito_sub, role_id FROM staff_members WHERE id = $1 AND tenant_id = $2`, [staffId, tenantId]
        );
        if (!staff) { res.status(404).json({ error: 'Staff member not found' }); return; }

        const permissions = await query(
            `SELECT p.id AS permission_id, p.category, p.display_name, p.description, p.is_sensitive,
                    CASE WHEN spo.granted IS NOT NULL THEN spo.granted WHEN rp.permission_id IS NOT NULL THEN TRUE ELSE FALSE END AS is_granted,
                    CASE WHEN spo.granted IS NOT NULL THEN 'override' WHEN rp.permission_id IS NOT NULL THEN 'role' ELSE 'none' END AS source
             FROM permissions p
...
router.put('/staff/:staffId/permissions', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        ...
        const staff = await queryOne(`SELECT id FROM staff_members WHERE id = $1 AND tenant_id = $2`, [staffId, tenantId]);
        if (!staff) { res.status(404).json({ error: 'Staff member not found' }); return; }

        await withTenant(tenantId, async (client) => {
            await client.query(`DELETE FROM staff_permission_overrides WHERE staff_id = $1 AND tenant_id = $2`, [staffId, tenantId]);
            for (const override of overrides) {
                if (!override.permission_id || typeof override.granted !== 'boolean') continue;
                await client.query(
                    `INSERT INTO staff_permission_overrides (staff_id, permission_id, tenant_id, granted, updated_by) VALUES ($1, $2, $3, $4, $5)`,
...
router.get('/my-permissions', async (req: Request, res: Response) => {
    try {
        ...
        if (userRole === 'owner' || userRole === 'admin' || userRole === 'superadmin') {
            ...
            res.json({ role: 'owner', role_display_name: 'Owner', permissions: allPerms.map((p: any) => p.id), is_owner: true });
            return;
        }
        ...
        if (!staff) {
            res.json({ role: userRole || 'staff', role_display_name: 'Staff', permissions: [], is_owner: false });
            return;
        }

        const perms = await query(`SELECT permission_id FROM get_effective_permissions($1, $2)`, [cognitoSub, tenantId]);
        res.json({
            role: (staff as any).role_name, role_display_name: (staff as any).role_display_name,
            permissions: perms.map((p: any) => p.permission_id), is_owner: false, staff_id: staff.id,
        });
    } catch (error: any) {
...
router.post('/bootstrap', checkPermission('manage_staff'), async (req: Request, res: Response) => {
    try {
        const tenantId = req.shopId!;
        const existing = await queryOne(`SELECT id FROM roles WHERE tenant_id = $1 LIMIT 1`, [tenantId]);
        if (existing) { res.json({ success: true, message: 'Roles already initialized', already_exists: true }); return; }
...
router.post('/staff/accept-invite', async (req: Request, res: Response) => {
    try {
        ...
        const staff = await queryOne(
            `UPDATE staff_members SET cognito_sub = $1, invite_status = 'accepted', last_login_at = NOW(), updated_at = NOW()
             WHERE invite_code = $2 AND invite_status = 'pending' RETURNING id, name, tenant_id`,
            [cognitoSub, invite_code.toUpperCase()]
        );
        if (!staff) { res.status(404).json({ error: 'Invalid or expired invite code' }); return; }
