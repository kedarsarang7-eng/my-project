...
//
// Endpoints:
//   POST /api/prescriptions          — Upload a prescription (Clinic side)
...
//   GET  /api/prescriptions/check/:rxId     — Check if already dispensed
//
// Auth: Firebase ID token (customerAuth) — both clinic and pharmacy users
// ============================================================================
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        // Check for duplicate rx_id
        const existing = await queryOne(
            'SELECT rx_id FROM shared_prescriptions WHERE rx_id = $1',
            [rx_id]
        );
        if (existing) {
            res.status(409).json({
                error: 'Prescription with this ID already exists',
                code: 'DUPLICATE_RX_ID',
            });
            return;
        }
...
router.get('/check/:rxId', async (req: Request, res: Response) => {
    try {
        ...
        const row = await queryOne<any>(
            'SELECT status FROM shared_prescriptions WHERE rx_id = $1',
            [rxId]
        );
...
router.patch('/:rxId/dispense', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        if (!pharmacy_shop_id) {
            res.status(400).json({
                error: 'pharmacy_shop_id is required',
                code: 'VALIDATION_ERROR',
            });
            return;
        }
        ...
        // Update
        await query(
            `UPDATE shared_prescriptions 
             SET status = 'dispensed', fulfilled_by = $1, fulfilled_at = NOW()
             WHERE rx_id = $2`,
