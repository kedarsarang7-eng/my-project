...
const { CognitoJwtVerifier } = require('aws-jwt-verify/cognito-verifier');;
...
function error(message, statusCode = 400, requestId) {
  return {
    statusCode,
    headers: {
      'Content-Type': 'application/json',
...
async function updateItem(tableName, key, updatesOrExpr, attrNames, attrValues) {
...
// Validation Helpers
function validateTenantAccess(userTenantId, requestedTenantId) {
...
function createPaginationResponse(items, limit, lastEvaluatedKey) {
  return {
    items,
    pagination: {
 ...
 */
function enforceTenantScope(data, userContext) {
 ...
 */
function withTenantCondition(keyCondition, tenantId) {
  return {
 ...
 */
function withTenantIsolation(handler) {
 ...
 */
function createConditionalUpdate(params) {
  const { 
    tableName, 
    key, 
    updates, 
 ...
 */
function withVersionIncrement(updates, currentVersion) {
  return {
    ...updates,
