// ============================================================================
// WebSocket Contract — mirrors my-backend/src/types/websocket.types.ts
// ============================================================================
// The Local_Backend serves the SAME real-time event contracts the AWS backend
// exposes (Requirement 4.3), via a Socket.io server bound to the loopback
// interface. The event names, client types, and event payload shape below are
// re-declared verbatim from the AWS WebSocket types so connected DukanX
// clients see an identical event surface in either mode.
// ============================================================================

/** Client types that can connect to the real-time channel. */
...
/** All supported real-time event names — identical to the AWS event set. */
...
/** Structure of a real-time event payload sent to clients — AWS-identical. */
export interface WSEvent {
    event: WSEventName;
    businessId: string;
    timestamp: string;
    data: Record<string, unknown>;
    /** Replay sequence number for missed-event recovery */
    seq?: number;
}

/** Handshake parameters expected when a client connects — mirrors WSConnectParams. */
...
/** Incoming message from a connected client — mirrors WSClientMessage. */
