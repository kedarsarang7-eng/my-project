...
class CleanupService {
  ...
  Future<Map<String, int>> runDailyCleanup() async {
    ...
    try {
      LoggerService.d(
        'CleanupService',
        'CleanupService: Starting daily cleanup...',
      );
      ...
      try {
        ...
        LoggerService.d(
          'CleanupService',
          'CleanupService: Database optimized (VACUUM)',
        );
      } catch (e) {
        LoggerService.d(
          'CleanupService',
          'CleanupService: VACUUM failed (harmless): $e',
        );
      }
    } catch (e) {
      LoggerService.d(
        'CleanupService',
        'CleanupService: Error during daily cleanup: $e',
      );
    ...
    await _db.transaction(() async {
      ...
      final logs =
          await (_db.select(_db.auditLogs)..where(
                (t) => t.timestamp.isSmallerOrEqual(Variable(cutoffDate)),
              ))
              .get();
      ...
      try {
        ...
        LoggerService.d(
          'CleanupService',
          'CleanupService: Archived $count logs to $filename',
        );

        // Delete from DB (Safe: already archived)
        await (_db.delete(
              _db.auditLogs,
            )..where((t) => t.timestamp.isSmallerOrEqual(Variable(cutoffDate))))
            .go();
      } catch (e) {
        LoggerService.d(
          'CleanupService',
          'CleanupService: Failed to archive logs: $e',
        );
  ...
  Future<int> _cleanSyncedQueueItems() async {
    ...
    final count =
        await (_db.delete(_db.syncQueue)..where(
              (t) =>
                  t.status.equals(SyncStatus.synced.value) &
                  t.lastAttemptAt.isSmallerOrEqual(Variable(cutoffDate)),
            ))
            .go();
    ...
    if (count > 0) {
      LoggerService.d(
        'CleanupService',
        'CleanupService: Cleaned $count old synced items',
      );
    }
    return count;
  }
  ...
  Future<int> _clearTempFiles() async {
    ...
    try {
      ...
      LoggerService.d(
        'CleanupService',
        'CleanupService: Error clearing temp files: $e',
      );
    }
    return count;
  }
