...
class SecureQrService {
  ...
  void _loadKeyFromEnv() {
    try {
      ...
      if (key != null && key.isNotEmpty) {
        _secretKey = key;
        _isInitialized = true;
        LoggerService.d('SecureQR', 'SecureQrService: HMAC key loaded from .env');
      }
    } catch (e) {
      // Fall back to default key - add QR_SIGNING_KEY to .env for production
      LoggerService.d('SecureQR', 'SecureQrService: Using default key');
    }
  }
  ...
  QrValidationResult validateQrPayload(String qrData) {
    try {
      ...
      LoggerService.d('SecureQR', 'SecureQrService: Validation error: $e');
