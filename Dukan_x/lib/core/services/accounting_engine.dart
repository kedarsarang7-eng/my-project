...
class AccountingEngine {
  FirebaseFirestore get _api => FirebaseFirestore.instance;
  ...
  Future<void> postTransaction({
    ...
    // Allow tiny floating point diff — should be 0 with paise arithmetic
    ...
    for (var item in items) {
      ...
      if (qtyChange != 0) {
        ...
        // A-04 FIX: Read current stock level to compute running balance.
        // This is a read-then-write pattern which is acceptable here because:
        // 1. The entire postTransaction is already batch-atomic
        // 2. Stock movements are append-only (conflicts resolved by reconciliation)
        double currentStock = 0;
        try {
          final productDoc = await _api
              .collection('businesses')
              .doc(businessId)
              .collection('products')
              .doc(item.itemId)
          ...
          if (productDoc.exists) {
            currentStock = (productDoc.data()?['quantity'] ?? 0).toDouble();
          }
        } catch (_) {
          // If product read fails, proceed with 0 — reconciliation will fix later
        }
  ...
  Future<List<LedgerEntry>> _generateLedgerEntries(
    ...
    // D. PAYMENT RECEIVED (Receipt) - Customer pays us
      ...
      // AE-02 FIX: Use explicit paymentMethod field if available, fall back to notes
      final paymentMethod = (txn.notes?.split('|').firstWhere(
        (s) => s.startsWith('method:'),
        orElse: () => '',
      ) ?? '').replaceFirst('method:', '').trim();
      ...
      if (paymentMethod == 'online' || paymentMethod == 'upi' ||
        ...
        // Legacy fallback: check notes for keywords (deprecated, remove after migration)
  ...
  Future<String> reverseTransaction({
    ...
    final entriesSnapshot = await _api
        .collection('businesses')
        .doc(businessId)
        .collection('ledger_entries')
    ...
    final stockMovesSnapshot = await _api
        .collection('businesses')
        .doc(businessId)
        .collection('stock_movements')
    ...
    final originalTxnRef = _api
        .collection('businesses')
        .doc(businessId)
        .collection('sales')
        .doc(originalTxnId);
    batch.update(originalTxnRef, {
      'isReversed': true,
  ...
  Future<bool> canModifyTransaction(
    ...
    final periodsSnapshot = await _api
        .collection('businesses')
        .doc(businessId)
