...
class RoleManagementService {
  ...
  /// Get all business_users records for a given user across all businesses.
  ///
  /// Returns every [BusinessUser] assignment where [userId] matches,
  /// supporting the multi-role/business picker when a user has more than
  /// one assignment.
  Future<List<BusinessUser>> getBusinessUsersForUser(String userId) async {
    final snapshot = await _firestore
        .collection('business_users')
  ...
  /// Watch a specific business_users document for real-time role changes.
  ///
  /// Returns a stream of [BusinessUser?] that emits whenever the document
  /// is created, updated, or deleted. Used by [SessionManager] to detect
  /// when an owner changes a staff member's role so the active session
  /// can update permissions without requiring logout/restart.
  ///
  /// Firestore snapshots() auto-reconnects when the device comes back online,
  /// satisfying the preservation requirement for offline graceful handling.
  Stream<BusinessUser?> watchBusinessUser(String businessId, String userId) {
    return _firestore
        .collection('business_users')
        .doc('${businessId}_$userId')
        .snapshots()
        .map((snapshot) {
          ...
          return BusinessUser.fromMap(snapshot.id, snapshot.data()!);
        });
  }
}
