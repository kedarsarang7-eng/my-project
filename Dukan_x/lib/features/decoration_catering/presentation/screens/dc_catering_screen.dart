          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Menu items, packages and meal planning',
    ...
    showDialog(
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setS) => AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(Icons.restaurant_menu_rounded, color: Color(0xFFD97706)),
              SizedBox(width: 8),
              Text('Add Menu Item'),
            ],
          ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                '${item.pricePerPlate.toStringAsFixed(0)}/plate',
      ...
      child: Row(
        children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    _chip(
                      '${package.menuItemIds.length} items',
          ...
          Column(
            ...
            children: [
              Text(
                '₹${package.pricePerPlate.toStringAsFixed(0)}',
                style: TextStyle(
                  fontSize: responsiveValue<double>(
                    context,
                    mobile: 18,
      ...
      data: (pkgs) {
        if (pkgs.isNotEmpty &&
            (_selectedPackageId == null ||
                !pkgs.any((p) => p.id == _selectedPackageId))) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) setState(() => _selectedPackageId = pkgs.first.id);
          });
        }
        final selectedPkg = pkgs.isEmpty
            ? null
            : pkgs.where((p) => p.id == _selectedPackageId).firstOrNull ??
                  pkgs.first;
        final guests = int.tryParse(_guestCtrl.text) ?? 200;
        final totalCost = selectedPkg != null
            ? selectedPkg.pricePerPlate * guests
            : 0.0;
                    ...
                    children: [
                      ...
                      if (selectedPkg != null) ...[
                        ...
                        menuAsync.when(
                          ...
                          data: (allItems) {
                            final pkgItems = allItems
                                .where(
                                  (m) => selectedPkg.menuItemIds.contains(m.id),
                                )
                                .toList();
                            return Column(
                              children: pkgItems
              ...
              SizedBox(
                ...
                child: Column(
                  children: [
                    _costCard(
                      'Per Plate',
                      '₹${selectedPkg?.pricePerPlate.toStringAsFixed(0) ?? '0'}',
                      const Color(0xFF2563EB),
                    ),
                    const SizedBox(height: 12),
                    _costCard(
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _rawMatRow(
                            'Rice',
                            '${(guests * 0.15).toStringAsFixed(0)} kg',
                          ),
                          _rawMatRow(
                            'Oil',
                            '${(guests * 0.05).toStringAsFixed(0)} L',
                          ),
                          _rawMatRow(
                            'Vegetables',
                            '${(guests * 0.2).toStringAsFixed(0)} kg',
                          ),
                          _rawMatRow(
                            'Gas Cylinders',
                            '${(guests ~/ 50 + 1)} pcs',
